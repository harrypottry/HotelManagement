...
public class Wechat {
    ...
    @Value("${weixin.sleepingbeauty.mpAppId}")
    private String sbAppId;

    @Value("${weixin.sleepingbeauty.mpAppKey}")
    private String sbAppKey;

    @Value("${weixin.hotelManagement.mpAppId}")
    private String hmAppId;

    @Value("${weixin.hotelManagement.mpAppKey}")
    ...
    public WechatToken mediaToken(String code, String appName) {
        if(appName.equals(HotelManagement)) {
            return tinyToken(code, hmAppId, hmAppKey);
        }else if(appName.equals(SleepingBeauty)){
            return tinyToken(code, sbAppId, sbAppKey);
        }
    ...
    public String getTicket() {
        ...
        if (ticket == null) {
            ...
            {
                String url = String.format(jsTokenUrl, sbAppId, sbAppKey);
                try {
                    String s = restTemplate.getForObject(url, String.class);
